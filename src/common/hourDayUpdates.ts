...
export function updateTokenDayData(token: Token, event: ethereum.Event): TokenDayData {
  ...
  tokenDayData.totalLiquidityETH = token.totalLiquidity.times(token.derivedIP as BigDecimal)
  tokenDayData.totalLiquidityUSD = tokenDayData.totalLiquidityETH.times(bundle.IPPriceUSD)
...
export function updateTokenHourData(token: Token, event: ethereum.Event): TokenHourData {
  ...
  let tokenPrice = token.derivedIP.times(bundle.IPPriceUSD)
  let isNew = false
  if (!tokenHourData) {
