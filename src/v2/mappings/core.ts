...
export function handleSync(event: Sync): void {
  ...
  bundle.IPPriceUSD = getEthPriceInUSD()
  bundle.save()

  token0.derivedIP = findEthPerToken(token0 as Token)
  token1.derivedIP = findEthPerToken(token1 as Token)
  token0.save()
  token1.save()

  // get tracked liquidity - will be 0 if neither is in whitelist
  let trackedLiquidityETH: BigDecimal
  if (bundle.IPPriceUSD.notEqual(ZERO_BD)) {
  ...
  pair.reserveETH = pair.reserve0
    .times(token0.derivedIP as BigDecimal)
    .plus(pair.reserve1.times(token1.derivedIP as BigDecimal))
  pair.reserveUSD = pair.reserveETH.times(bundle.IPPriceUSD)

  // use tracked amounts globally
  uniswap.totalLiquidityETH = uniswap.totalLiquidityETH.plus(trackedLiquidityETH)
  uniswap.totalLiquidityUSD = uniswap.totalLiquidityETH.times(bundle.IPPriceUSD)
...
export function handleMint(event: Mint): void {
  ...
  let bundle = Bundle.load('1')!
  let amountTotalUSD = token1.derivedIP
    .times(token1Amount)
    .plus(token0.derivedIP.times(token0Amount))
    .times(bundle.IPPriceUSD)
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountETH = token1.derivedIP
    .times(amount1Total)
    .plus(token0.derivedIP.times(amount0Total))
    .div(BigDecimal.fromString('2'))
  let derivedAmountUSD = derivedAmountETH.times(bundle.IPPriceUSD)
  ...
  token0DayData.dailyVolumeETH = token0DayData.dailyVolumeETH.plus(amount0Total.times(token0.derivedIP as BigDecimal))
  token0DayData.dailyVolumeUSD = token0DayData.dailyVolumeUSD.plus(
    amount0Total.times(token0.derivedIP as BigDecimal).times(bundle.IPPriceUSD)
  )
  token0DayData.save()

  // swap specific updating
  token1DayData.dailyVolumeToken = token1DayData.dailyVolumeToken.plus(amount1Total)
  token1DayData.dailyVolumeETH = token1DayData.dailyVolumeETH.plus(amount1Total.times(token1.derivedIP as BigDecimal))
  token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(
    amount1Total.times(token1.derivedIP as BigDecimal).times(bundle.IPPriceUSD)
  )
  token1DayData.save()
}
